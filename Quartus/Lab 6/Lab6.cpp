...
int PushButton(char *pBase,int value);
...
//state = RegisterRead(pBase,KEY_OFFSET)
  if(value == 1){ 
  WriteAllLeds(pBase,RegisterRead(pBase,LEDR_OFFSET) + 1);
  cout << "Key 0 Pressed, LEDS incremented "<<endl; 
  }
  if(value == 2){ 
  WriteAllLeds(pBase,RegisterRead(pBase,LEDR_OFFSET) - 1);
  cout << "Key 1 Pressed, LEDS incremented "<<endl; 
  }
  if(value == 4){ 
   WriteAllLeds(pBase,RegisterRead(pBase,LEDR_OFFSET) >> 1);
  cout << "Key 2 Pressed, LEDS incremented "<<endl; 
  }
  if(value == 8){ 
    WriteAllLeds(pBase,RegisterRead(pBase,LEDR_OFFSET) << 1);
  cout << "Key 3 Pressed, LEDS incremented "<<endl; 
  }
  //else if(state == 4){
  //val = num & RegisterRead(pBase,KEY_OFFSET)
  //RegisterWrite(pBase,LEDR_OFFSET,val);
}


int ButtonCheck(char *pBase){
  int prevState = PushButtonGet(pBase);

  while(1){
    int curS = PushButtonGet(pBase);
    if(curS != prevState){
      PushButton(pBase, curS);
      prevState = curS;
   }
   else if(curS > 8){
   break;
